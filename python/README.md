...
- **ライブラリのインポート**  
  必要なライブラリとして,Flask,os,boto3（AWS S3 用）,OpenCV (cv2),DeepFace,SQLAlchemy（ただし現状このコード内では利用していない）,numpy,base64 をインポートしている．  
  これらのライブラリによって,画像処理,感情解析,S3 操作,そしてデータベース接続（将来的な利用を想定している可能性がある）などの処理を実現している．
